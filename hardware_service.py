...
Supports SIMULATE mode for running without hardware (auto-enabled on Windows).
"""
...
# ── Simulation mode: auto-detect Windows or honour env var ──
SIMULATE = (
...
def init(app, config):
    ...
    if SIMULATE:
        _logger.info("[HARDWARE] *** SIMULATION MODE *** — no real ESP32 serial")
        return
...
def _sim_response(cmd: str) -> dict:
    """Generate a simulated ESP32 response for the given command."""
    cu = cmd.upper()
    if "S,PING" in cu:
        return {"ok": True, "response": "S,OK", "cmd": cmd}
    if "C,TARE" in cu:
        return {"ok": True, "response": "C,TARE,OK", "cmd": cmd}
    if "C,DZ" in cu:
        return {"ok": True, "response": "DZ,OK", "cmd": cmd}
    if "C,DS" in cu:
        return {"ok": True, "response": "C,DS,OK", "cmd": cmd}
    if "C,LOAD" in cu:
        return {"ok": True, "response": "C,LOAD,OK", "cmd": cmd}
    if "T,DIM" in cu:
        dim = round(random.uniform(5.0, 15.0), 2)
        return {"ok": True, "response": f"D,DIM,{dim}", "cmd": cmd}
    if "T,HARD" in cu:
        hard = round(random.uniform(20.0, 200.0), 2)
        return {"ok": True, "response": f"D,HARD,{hard}", "cmd": cmd}
    if "T,HOME" in cu:
        return {"ok": True, "response": "T,HOME,OK", "cmd": cmd}
    if "T,BO" in cu:
        return {"ok": True, "response": "T,BO,OK", "cmd": cmd}
    if "V,L,1" in cu:
        return {"ok": True, "response": "V,L,1,OK", "cmd": cmd}
    if "V,L,0" in cu:
        return {"ok": True, "response": "V,L,0,OK", "cmd": cmd}
    # Fallback: generic OK
    return {"ok": True, "response": "OK", "cmd": cmd}
...
def send_command(cmd: str, timeout=COMMAND_TIMEOUT, max_retries=MAX_RETRIES):
    ...
    if SIMULATE:
        time.sleep(random.uniform(0.05, 0.15))  # small realistic delay
        result = _sim_response(cmd)
        if _logger:
            _logger.debug("[HARDWARE][SIM] %s -> %s", cmd, result.get("response"))
        return result
...
def wait_for_line_containing(pattern: str, timeout: float = 60.0) -> dict:
    ...
    if SIMULATE:
        ...
        return {"ok": True, "response": pattern}
        ...
        def sim_generate():
            ...
            while True:
                try:
                    ...
                    break
        return Response(sim_generate(), mimetype='text/event-stream')
