...
function goToPage(pageName) {
    if (typeof window.closeOSK === 'function') {
        window.closeOSK();
    }
    ...
    if (_navigatingAfterAbort) {
        ...
        typeof isValCalHubPage === 'function' &&
        isValCalHubPage(pageName) &&
        typeof isValidationOrCalibrationHardwareActive === 'function' &&
        !isValidationOrCalibrationHardwareActive()
    ) {
        if (typeof softClearValCalRunningFlags === 'function') softClearValCalRunningFlags();
    } else if (isOperationRunning()) {
        var activeForOpCheck = document.querySelector('.page.active');
        var activePageId = activeForOpCheck ? activeForOpCheck.id : '';
        var targetPageId = 'page-' + pageName;
        if (activePageId && activePageId === targetPageId) {
            console.log('[DEBUG] goToPage: already on', pageName, '- skipping duplicate navigation');
            return;
        }
    ...
    if (pageName === 'validate' && typeof refreshSessionUserFromServer === 'function') {
    ...
    if (pageName === 'validate' && typeof refreshValidatePageAccess === 'function') {
    ...
    if (activePage && (activePage.id === 'page-load-calibration' || activePage.id === 'page-distance-zero-calibration')) {
        if (pageName !== 'load-calibration' && pageName !== 'distance-zero-calibration') {
            if (typeof resetDistanceCalibSession === 'function') resetDistanceCalibSession();
        }
    }
    ...
    const userProfileEl = document.querySelector('.sidebar .user-profile');
    if (userProfileEl) {
    ...
    // Auto-load member list when navigating to manage / locked / disabled profiles
    if (pageName === 'manage-members' || pageName === 'locked-members' || pageName === 'disabled-members') {
        setTimeout(() => {
            if (typeof loadMembersAndRender === 'function') {
                loadMembersAndRender();
            } else if (typeof displayMembersList === 'function') {
                displayMembersList();
            }
        }, 50);
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (typeof initAppSessionOnLoad === 'function') {
        initAppSessionOnLoad().finally(function () {
            if (typeof window.closeOSK === 'function') {
                window.closeOSK();
            }
        });
    ...
    if (typeof loadBiometricSetting === 'function') {
...
function logout() {
    ...
    var doLogout = function () {
        var abortP = (typeof abortPendingReportOnLogout === 'function')
...
async function apiRequest(endpoint, options = {}) {
    try {
        ...
        if (typeof window !== 'undefined' && window.currentUser) {
            let hdrRole = window.currentUser.role;
            if (!hdrRole && typeof getCurrentRole === 'function') {
                const gr = getCurrentRole();
                if (gr) hdrRole = gr;
            }
...
async function saveReport(reportData) {
    try {
        const report = {
            ...
            testData: Object.assign({}, reportData)
        };
        if (reportData.recipe) report.recipe = reportData.recipe;
        if (reportData.statistics) report.statistics = reportData.statistics;
        if (typeof stampOperatorOnTestReportPayload === 'function') {
            stampOperatorOnTestReportPayload(report);
        }
        const result = await apiRequest('/api/data/reports', {
            method: 'POST',
            body: report
...
async function populateReportPreviewDom(stored) {
    ...
    var td = (stored && stored.testData) ? stored.testData : (stored || {});
    var operatorName = (stored && (stored.operatorName || td.operatorName)) || td.operatedBy || '';
    if (!operatorName) {
        operatorName = (typeof currentUser !== 'undefined' && currentUser && currentUser.name)
            ? currentUser.name
            : (window.currentUser && window.currentUser.name ? window.currentUser.name : '');
    }
    var employeeId = (stored && (stored.employeeId || td.employeeId || td.operatorId)) || '';
    if (!employeeId) {
        employeeId = (typeof currentUser !== 'undefined' && currentUser && currentUser.username)
            ? currentUser.username
            : (window.currentUser && window.currentUser.username ? window.currentUser.username : '');
    }
    ...
    if (apprRemEl) {
        var apprRem = stored && stored.approvalRemarks;
        apprRemEl.textContent = (apprRem != null && String(apprRem).trim() !== '') ? apprRem : 'N/A';
    }
...
function resolveCreatedReportId(data) {
    ...
    if (data.report && data.report.id != null) return data.report.id;
    return null;
}
    ...
    return apiRequest('/api/reports/' + reportId + '/preview').then(async function (data) {
        if (data && data.preview) {
            ...
            setTimeout(function () {
                ...
                if (typeof isReportPendingApproval === 'function' &&
                    isReportPendingApproval(data.preview) &&
                    typeof scrollReportApprovePanelIntoView === 'function') {
...
async function saveRecipe() {
    ...
    if (window._recipeSaveInFlight) return null;

    const editId = editingRecipeId;
    const url = editId ? `/api/data/recipes/${editId}` : '/api/data/recipes';
    const method = editId ? 'PUT' : 'POST';
    var role = (typeof getCurrentRole === 'function' ? String(getCurrentRole() || '').toLowerCase() : '');

    async function persistRecipe(approvalToken) {
        window._recipeSaveInFlight = true;
        try {
            const headers = {};
            if (approvalToken) headers['X-Approval-Verify-Token'] = approvalToken;
            const result = await apiRequest(url, { method: method, headers: headers, body: recipe });
            if (editId) editingRecipeId = null;
            const saved = result.recipe || result;
            console.log('Recipe saved:', saved);
            var apprSt = (typeof getEffectiveRecipeApprovalStatus === 'function' && saved)
                ? getEffectiveRecipeApprovalStatus(saved) : 'pending';
            if (typeof showAppModal === 'function') {
                if (role === 'factory' || apprSt === 'approved') {
    ...
    try {
        if (role === 'factory') {
            return await persistRecipe(null);
        }
        if (typeof openApprovalVerifyModal !== 'function' || typeof _approvalVerifyModalOptionsForRecipe !== 'function') {
            if (typeof showAppModal === 'function') {
        ...
        var token;
        try {
            token = await openApprovalVerifyModal(_approvalVerifyModalOptionsForRecipe());
        } catch (modalErr) {
            var modalMsg = (modalErr && modalErr.message) ? String(modalErr.message) : 'Approval verification failed.';
            if (typeof showAppModal === 'function') {
        ...
        return await persistRecipe(token);
    } catch (e) {
...
async function displayRecipeList() {
    ...
    if (recipeListLoadOnly && typeof getEffectiveRecipeApprovalStatus === 'function') {
        recipes = recipes.filter(function (r) { return getEffectiveRecipeApprovalStatus(r) === 'approved'; });
        if (!recipes.length) {
            container.innerHTML = '<p class="empty-state-message">No approved recipes available.</p>';
            return;
        }
    }
        ...
        <table class="recipe-table">
            <thead>
                <tr>
                    ...
                    <th>Weight</th>` +
        (showLoad ? '<th>Load</th>' : '') +
        (showActions ? '<th>Approval</th><th class="actions-col">Actions</th>' : '') + `
                </tr>
            </thead>
            <tbody>
    `;
    ...
    recipes.forEach(recipe => {
        ...
        const apprStatus = (typeof getEffectiveRecipeApprovalStatus === 'function')
        ...
        (showActions ? '<td>' + apprLabel + '</td><td class="actions-cell actions-col">' + actionsBtnHtml + '</td>' : '') + `
            </tr>
        `;
    });
...
function openRecipeActionsModal(recipeId) {
    ...
    if (apprBtn) {
        var apprSt = (recipe && typeof getEffectiveRecipeApprovalStatus === 'function')
            ? getEffectiveRecipeApprovalStatus(recipe) : '';
        var showAppr = !!(recipe && apprSt === 'pending' && typeof userCanApproveByQaRule === 'function' && userCanApproveByQaRule());
...
function submitRecipeApprove() {
    ...
    var openModal = (typeof openApprovalVerifyModal === 'function')
        ? openApprovalVerifyModal(_approvalVerifyModalOptionsForRecipe())
        : Promise.resolve(null);
    openModal.then(function (token) {
        if (!token) return;
        return apiRequest('/api/data/recipes/' + id + '/approve', {
            method: 'POST',
...
function confirmBatchNumberAndLoad() {
    ...
    if (typeof getEffectiveRecipeApprovalStatus === 'function' &&
        getEffectiveRecipeApprovalStatus(pendingRecipeToLoad) === 'pending') {
        if (typeof showAppModal === 'function') {
...
function handleTestReportSavedNavigation(reportId) {
    try {
        ...
        console.error('Post-test navigation failed:', e);
        if (reportId && typeof openReportPreview === 'function') {
            openReportPreview(reportId, { setGate: true });
        }
    }
...
async function initFactorySettings() {
    try {
        ...
        if (pwdDaysEl) pwdDaysEl.value = String(settings.passwordResetPeriodDays != null ? settings.passwordResetPeriodDays : 30);
        var autoEl = document.getElementById('factory-auto-logout-minutes');
        if (autoEl) autoEl.value = String(settings.autoLogoutMinutes != null ? settings.autoLogoutMinutes : 0);
        var bioEl = document.getElementById('factory-biometric-enabled');
        if (bioEl) bioEl.value = (settings.biometricEnabled === false || settings.biometricEnabled === 'disabled') ? 'disabled' : 'enabled';
        if (typeof applyFactoryAutoLogoutSetting === 'function') applyFactoryAutoLogoutSetting(settings);
...
/* ===== CFR compliance (ported from Friability/TapDensity shell) ===== */
...
function normalizeBiometricEnabled(v) {
    if (v === false || v === 0 || v === '0' || v === 'disabled' || v === 'off' || v === 'no') return false;
    return true;
}
...
function applyBiometricSetting(enabled) {
    ...
    if (loginDivider) loginDivider.style.display = biometricEnabledSetting ? '' : 'none';
    if (loginBtn) {
...
function performAutoLogoutDueToInactivity() {
    ...
    var req = (typeof apiRequest === 'function')
        ...
        : fetch(base + '/api/data/auth/logout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason: 'inactivity' }) });
...
function canViewAuditLog() {
    ...
    if (!u) return false;
    if (String(u.role || '').toLowerCase() === 'factory') return true;
    if (typeof canAccess === 'function') return canAccess(u, 'audit-view');
    return false;
}
