#!/usr/bin/env python3
"""
Run all Hardness ESP commands and print the ESP↔Pi communication log.
Maps actual UART lines against hardness_firmware.txt.
...
def _listen_stream(seconds, label="stream"):
    ...
    while time.time() < deadline:
        ...
        for line in lines:
            parsed = hw.parse_hardness_progress_line(line)
...
def main():
    parser = argparse.ArgumentParser(description="Probe Hardness ESP UART commands")
    ...
    if args.validation:
        ...
        start_fn = hw.cmd_start_hardness
    ...
    if serial_ok:
        ...
        if args.with_pause:
            print("3) pause*")
            r = hw.cmd_pause_hardness()
            print("   ", r)
            time.sleep(1.0)
            print("4) resume*")
            r = hw.cmd_resume_hardness()
            print("   ", r)
            _listen_stream(half, "after resume")

        print("5) stop*")
        r = hw.cmd_stop_hardness()
        print("   ", r)
        if not r.get("ok"):
            exit_code = 1
    else:
