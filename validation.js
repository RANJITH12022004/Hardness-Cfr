...
var VAL_CAL_HUB_PAGES = [
    ...
    'distance-validation',
    'load-calibration',
    'distance-zero-calibration'
];

function isValCalHubPage(pageName) {
    return VAL_CAL_HUB_PAGES.indexOf(pageName) !== -1;
}

function isDistanceCalibrationHardwareActive() {
    return (
...
/** True only while hardware/API work is in flight (not idle UI on val/cal screens). */
function isValidationOrCalibrationHardwareActive() {
    return !!(
        loadValidationRunning ||
        distanceValidationRunning ||
        loadCalibrationRunning ||
        distanceCalibRunning ||
        isDistanceCalibrationHardwareActive()
    );
}

function isValidationOrCalibrationRunning() {
    return isValidationOrCalibrationHardwareActive();
}

function softClearValCalRunningFlags() {
...
function prepareValCalNavigation() {
...
function navigateValCalHub(pageName) {
    if (!isValidationOrCalibrationHardwareActive()) {
        softClearValCalRunningFlags();
    }
    prepareValCalNavigation();
    if (typeof goToPage === 'function') goToPage(pageName);
}

function navigateToValidateHub() {
    navigateValCalHub('validate');
}

function resetDistanceValidationSession() {
    distanceValidationRunning = false;
...
function resetDistanceCalibSession() {
    distanceCalibRunning = false;
...
/** Leave calibration screen without the global operation-exit modal (DUMMY-style). */
function abortCalibrationAndGoBack(targetPage) {
    targetPage = targetPage || 'calibration-type-select';
    var hardwareActive = !!(
        loadCalibrationRunning ||
        distanceCalibRunning ||
        distanceCalibState === CALIB_ZEROING ||
...
function _userCanRunValidation() {
    var u = window.currentUser;
    if (!u) return false;
    if (typeof canAccess === 'function') return canAccess(u, 'validation-test');
    if (typeof userHasInternalKey === 'function') return userHasInternalKey(u, 'validation-test');
    return false;
}

function _userHasPermCard(user, cardKey) {
    if (!user || !cardKey) return false;
    var raw = user.featureOverrides;
    if (!raw || !Array.isArray(raw.allow)) return false;
    return raw.allow.indexOf(cardKey) !== -1;
}

function _userCanRunCalibration() {
    var u = window.currentUser;
    if (!u) return false;
    if (typeof isFactoryLikeRole === 'function' && isFactoryLikeRole(String(u.role || '').toLowerCase(), u)) {
        return true;
    }
    if (_userHasPermCard(u, 'perm_calibration_test')) return true;
    if (typeof canAccess === 'function') return canAccess(u, 'calibration-menu');
    if (typeof userHasInternalKey === 'function') return userHasInternalKey(u, 'calibration-menu');
    return false;
}
...
function selectOperation(operation) {
    var run = function () {
        if (operation === 'validate') {
            ...
            resetDistanceValidationSession();
            resetDistanceCalibSession();
            navigateValCalHub('calibration-type-select');
        }
    };
    if (typeof refreshSessionUserFromServer === 'function') {
        refreshSessionUserFromServer().then(run);
    } else {
        run();
    }
}
...
function completeLoadValidation() {
    ...
    fetch('/api/data/reports', {
        ...
        .then(function (r) { return r.json(); })
        .then(function (data) {
            var reportId = typeof resolveCreatedReportId === 'function'
